      ...
      <div class="space-y-2">
        <transition-group name="list-transition">
          <div
            ...
            :key="key"
          >
            <label class="text-sm text-sm text-gray-300" :for="key">{{ key }}</label>
...
export default {
  ...
  setup(props) {
    ...
    function addAttribute() {
      if (state.key === '' || blackListAttrs.includes(state.key.toLowerCase()) || state.attributes[state.key]) return;
